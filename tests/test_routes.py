...
def test_order_a_pizza():
    ...
    response = app.test_client().post('/order-a-pizza', json = data)
    parsed_response = (json.loads(response.data))
...
def test_order_a_drink():
    ...
    response = app.test_client().post('/order-a-drink', json = data)
    parsed_response = (json.loads(response.data))
...
def test_change_an_order():
    ...
    response = app.test_client().post('/change-an-order', json = data)
    parsed_response = (json.loads(response.data))
...
1, 'size': 'L', 'toppings': {'mushrooms': 1, 'olives': 2, 'tomatoes': 1}, 'type': 'pepperoni'}], 'price': 33.5}

def test_get_full_menu():
    response = app.test_client().get('/get-full-menu')
    parsed_response = (json.loads(response.data))
    assert parsed_response == {"pizza": {"size": {"S": 0.8, "M": 1.0, "L": 1.5}, "type": {"pepperoni": 8, "margherita": 6, "vegetarian": 14.5, "Neapolitan": 9}, "topping": {"olives": 3, "tomatoes": 1, "mushrooms": 2, "jalapenos": 6, "chicken": 2, "beef": 4.5, "pepperoni": 2.5}}, "drink": {"Coke": 2, "Diet Coke": 3, "Coke Zero": 4, "Pepsi": 2, "Diet Pepsi": 1, "Dr. Pepper": 3, "Water": 4, "Juice": 2}}

def test_get_price_for_specific_item():
    data = {"item": "Coke"}
    response = app.test_client().post('/get-price-for-specific-item', json = data)
...
def test_add_new_type():
    ...
    parsed_response = (json.loads(response.data))
    assert parsed_response == {'drink': {'Coke': 2, 'Coke Zero': 4, 'Diet Coke': 3, 'Diet Pepsi': 1, 'Dr. Pepper': 3, 'Juice': 2, 'Pepsi': 2, 'Water': 4}, 'pizza': {'size': {'L': 1.5, 'M': 1.0, 'S': 0.8}, 'topping': {'beef': 4.5, 'chicken': 2, 'jalapenos': 6, 'mushrooms': 2, 'olives': 3, 'pepperoni': 2.5, 'tomatoes': 1}, 'type': {'Neapolitan': 9, 'New': 47.0, 'margherita': 6, 'pepperoni': 8, 'vegetarian': 14.5}}}

def test_set_address():
    data = {"order_number": 4, "address": "333 Street"}
    response = app.test_client().post('/set-address', json = data)
    parsed_response = (json.loads(response.data))
    assert parsed_response == {"order_number": 4, "pizzas": [], "drinks": [], "address": "333 Street", "price": 0}


def test_cancel_order():
    data = {"order_number": 4}
    response = app.test_client().post('/cancel-order', json = data)
    parsed_response = (json.loads(response.data))
    assert parsed_response == [{'order_number': 1, 'address': '100 Street', 'drinks': [{'drink_name': 'Pepsi', 'item_id': 1, 'number': 10}], 'pizzas': [{'item_id': 1, 'number': 1, 'size': 'L', 'toppings': {'mushrooms': 1, 'olives': 2, 'tomatoes': 1}, 'type': 'pepperoni'}], 'price': 33.5}, {'order_number': 2, 'address': '', 'drinks': [{'drink_name': 'Diet Coke', 'item_id': 1, 'number': 5}], 'pizzas': [{'item_id': 1, 'number': 1, 'size': 'L', 'toppings': {'beef': 2, 'chicken': 3, 'tomatoes': 1}, 'type': 'margherita'}], 'price': 39.0}, {'order_number': 3, 'address': '200 Street', 'drinks': [{'drink_name': 'Juice', 'item_id': 1, 'number': 2}], 'pizzas': [{'item_id': 1, 'number': 1, 'size': 'L', 'toppings': {'beef': 2, 'chicken': 3, 'tomatoes': 1}, 'type': 'margherita'}], 'price': 28.0}]

def test_change_price_for_item():
    data = {"item": "olives", "price": 5}
    response = app.test_client().post('/change-price-for-item', json = data)
    parsed_response = (json.loads(response.data))
    assert parsed_response == {'drink': {'Coke': 2, 'Coke Zero': 4, 'Diet Coke': 3, 'Diet Pepsi': 1, 'Dr. Pepper': 3, 'Juice': 2, 'Pepsi': 2, 'Water': 4}, 'pizza': {'size': {'L': 1.5, 'M': 1.0, 'S': 0.8}, 'topping': {'beef': 4.5, 'chicken': 2, 'jalapenos': 6, 'mushrooms': 2, 'olives': 5, 'pepperoni': 2.5, 'tomatoes': 1}, 'type': {'Neapolitan': 9, 'New': 47.0, 'margherita': 6, 'pepperoni': 12, 'vegetarian': 14.5}}}
